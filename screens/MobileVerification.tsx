...
const MobileVerification: React.FC = () => {
  ...
  const handleVerifyOtp = async (): Promise<void> => {
    ...
    try {
      ...
      if (!userDoc.exists) {
        // Create new user in Firestore with default name fields
        ...
        // Get referral code from AsyncStorage if available
        const referralCode = await AsyncStorage.getItem('pendingReferralCode');
        if (referralCode) {
          // Apply the referral code
          try {
            const referralResult = await applyReferralCode(user.uid, referralCode);
            console.log('Referral code application result:', referralResult);
        ...
        // After creating the user, navigate to a profile completion page
        // For now, we'll just redirect to home but you could add a profile completion page
      } else {
        // Update last login
